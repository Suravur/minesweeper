...
grid = [].concat.apply([], gridMatrix); // (B) CREATE HTML TABLE STRING

var perrow = width,
    html = "<table><tr>"; // LOOP THROUGH ARRAY AND ADD TABLE CELLS

for (var i = 0; i < grid.length; i++) {
  // "NORMAL" CELL
  switch (grid[i]) {
    case -1:
      html += "<td style=\"color:black\" id=".concat(i, ">\u2022</td>");
      break;

    case 0:
      html += "<td style=\"color:white\" id=".concat(i, ">&nbsp</td>");
      break;

    case 1:
      html += "<td style=\"color:blue\" id=".concat(i, ">").concat(grid[i], "</td>");
      break;

    case 2:
      html += "<td style=\"color:green\" id=".concat(i, ">").concat(grid[i], "</td>");
      break;

    case 3:
      html += "<td style=\"color:red\" id=".concat(i, ">").concat(grid[i], "</td>");
      break;

    case 4:
      html += "<td style=\"color:darkblue\" id=".concat(i, ">").concat(grid[i], "</td>");
      break;

    case 5:
      html += "<td style=\"color:maroon\" id=".concat(i, ">").concat(grid[i], "</td>");
      break;

    case 6:
      html += "<td style=\"color:darkcyan\" id=".concat(i, ">").concat(grid[i], "</td>");
      break;

    case 7:
      html += "<td style=\"color:black\" id=".concat(i, ">").concat(grid[i], "</td>");
      break;

    case 8:
      html += "<td style=\"color:darkgray\" id=".concat(i, ">").concat(grid[i], "</td>");
      break;
...
html += "</tr></table>"; // (C) ATTACH HTML TO CONTAINER
